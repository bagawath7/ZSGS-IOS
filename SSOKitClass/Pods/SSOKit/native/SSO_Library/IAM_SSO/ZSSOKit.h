...
+(void)generateHandshakeIDHavingClientZID:(NSString *)clientZID handler:(ZSSOKitHandShakeIDHandler)handshakeIDHandler;

+(void)generateHandshakeIDHavingClientZID:(NSString *)clientZID havingZUID:(NSString *)zuid handler:(ZSSOKitHandShakeIDHandler)handshakeIDHandler;
...
/**
 ...
 Call this method for showing the Accounts web Close Account page on SFSafari. Note: For now, it will prefill the email id, and then further authentications has to be done once and then during close account, again passwrod would be prompted. There will not be any callback after closeaccount. Only when the user taps on Done in SFSafari, you will get Close Account dismissed k_SSOCloseAccountDismissedError error. So, when you get this error in the callback, call the "checkAndLogoutUserDuringInvalidOAuth" method and handle the returned error and please take the user out of the app accordingly!
@param closeaccountHandler handler block.
 */
+ (void) presentCloseAccountViewControllerWithSignInPage:(ZSSOKitErrorResponse) closeaccountHandler NS_EXTENSION_UNAVAILABLE_IOS("");

/**
 
 For MULTI-ACCOUNT
 Call this method for showing the Accounts web Close Account page on SFSafari. Note: For now, it will prefill the email id, and then further authentications has to be done once and then during close account, again passwrod would be prompted. There will not be any callback after closeaccount. Only when the user taps on Done in SFSafari, you will get Close Account dismissed k_SSOCloseAccountDismissedError error. So, when you get this error in the callback, call the "checkAndLogoutUserDuringInvalidOAuth" method and handle the returned error and please take the user out of the app accordingly!
@param zuid ZUID of the respective user.
@param closeaccountHandler handler block with an NSError arugument
 
 */

+ (void) presentCloseAccountViewControllerWithSignInPageForZUID:(NSString *)zuid handler:(ZSSOKitErrorResponse) closeaccountHandler NS_EXTENSION_UNAVAILABLE_IOS("");
