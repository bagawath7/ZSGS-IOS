...
+ (void) presentManageAccountsViewController:(ZSSOKitManageAccountsHandler)manageHandler{
    ...
    [[ZIAMUtil sharedUtil] presentManageAccountsViewControllerWithSuccess:^(NSString *accessToken, BOOL changed, ZSSOUser *zUser) {
        manageHandler(accessToken,changed,zUser,nil);
    } andFailure:^(NSError *error) {
        manageHandler(nil,NO,nil,error);
    }];
...
+(NSArray *) getZSSOUsersArray{
    return [[ZIAMUtil sharedUtil] getZSSOUsersArray];
}
...
+(void)generateHandshakeIDHavingClientZID:(NSString *)clientZID handler:(ZSSOKitHandShakeIDHandler)handshakeIDHandler{
    [[ZIAMUtil sharedUtil] generateHandshakeIDHavingClientZID:clientZID WithSuccess:^(NSString *token) {
...
+(void)generateHandshakeIDHavingClientZID:(NSString *)clientZID havingZUID:(NSString *)zuid handler:(ZSSOKitHandShakeIDHandler)handshakeIDHandler{
    [[ZIAMUtil sharedUtil] generateHandshakeIDHavingClientZID:clientZID havingZUID:zuid WithSuccess:^(NSString *token) {
         handshakeIDHandler(token, nil);
    } andFailure:^(NSError *error) {
        handshakeIDHandler(nil, error);
    }];
...
+ (void) presentCloseAccountViewControllerWithSignInPage:(ZSSOKitErrorResponse) closeaccountHandler{
    [self presentCloseAccountViewControllerWithSignInPageForZUID:[[ZIAMUtil sharedUtil] getCurrentUserZUIDFromKeychain] handler:closeaccountHandler];
}

+ (void) presentCloseAccountViewControllerWithSignInPageForZUID:(NSString *)zuid handler:(ZSSOKitErrorResponse) closeaccountHandler{
    [[ZIAMUtil sharedUtil] presentCloseAccountForZUID:zuid failure:^(NSError *error) {
        closeaccountHandler(error);
    }];
}
